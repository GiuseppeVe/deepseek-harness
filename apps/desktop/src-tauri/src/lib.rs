...
pub fn resolve_desktop_paths<F>(data_directory: F, _source_dsh_cli: &Path) -> DesktopPaths
where
    F: FnOnce() -> PathBuf,
{
...
mod tests {
    ...
    fn resolves_every_directory_under_private_app_data() {
        ...
        let paths = resolve_desktop_paths(|| app_data.clone(), Path::new("C:/fixture/dsh.ts"));
