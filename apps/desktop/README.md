...
Before Desktop creates mutable descendants it creates this root, opens every existing item without following reparse points, rejects reparse points, and resets the root and every existing descendant DACL to the current Windows SID through that same handle. The fixed noninteractive PowerShell program receives the root only through process environment; a DACL failure stops startup.

The lease records the authenticated child PID, apply-lifetime nonce, Windows creation FILETIME, and control token. Electron keeps the nonce and token out of CLI arguments, logs, diagnostics, preload, and renderer IPC. A fresh child must report its spawned PID through the identity route before lease write; an exit or ready identity mismatch fails startup without publishing a lease. Electron can quiesce that still-direct child only before ownership publication. Reattach and later force termination revalidate the exact authenticated identity and current FILETIME.

## Window and lifecycle

The sole BrowserWindow enables `contextIsolation` and web security, disables Node integration and webviews, loads only the loopback origin, and denies external windows plus non-loopback navigation or redirects. Preload exposes only frozen `backend.status`, `backend.restart`, and `desktop.requestClose` operations when Electron runs the Desktop-origin main frame. IPC requires the same WebContents, same main frame, and exact loopback origin.
