...
describe('renderConfigDump', () => {
  ...
  it('upserts canonical root rows at first existing positions and appends absent rows', () => {
    const dir = tmp()
    const base = join(dir, 'base.yml')
    writeFileSync(base, [
      '- id: before',
    ...
    const dump = renderConfigDump(NAME, base, [{
      ...
      patches: [{
        upsert: [{
          id: 'desktop-control',
          name: './canonical-control.mjs',
          config: { source: 'desktop' },
        }, {
          id: 'desktop-status',
          name: './desktop-status.mjs',
        }],
      }],
    }], () => {})
    const parsed = yaml.load(dump, { schema: entryListSchema }) as { id: string; name: string }[]
    expect(parsed.map(entry => `${entry.id}:${entry.name}`)).toEqual([
      'before:./noop.mjs',
      'desktop-control:./canonical-control.mjs',
      'between:./noop.mjs',
      'desktop-status:./desktop-status.mjs',
    ])
  })
  ...
  it('rejects malformed, mixed, or sibling-operation upserts before rendering', () => {
    ...
    expect(() => renderConfigDump(NAME, base, [{
      label: 'mixed.yml',
      patches: [{
        insert: [{ id: 'inserted', name: './noop.mjs' }],
        upsert: [{ id: 'canonical', name: './noop.mjs' }],
      }],
    }], () => {})).toThrow('patch cannot contain both insert and upsert')
    expect(() => renderConfigDump(NAME, base, [{
      label: 'nested.yml',
